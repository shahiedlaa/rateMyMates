...
export class TableRankingComponent {
  ...
  ngOnInit() {
    this.subscription = this.teamviewService.dashboardEmitter.subscribe(
      (element) => {
        if (element && this.tableData) {
          setTimeout(() => {
            this.generateTableData(this.tableData);
            this.generateMatchHistoryTable(this.tableData);
  ...
  generateTableData(data: any) {
    ...
    uniquePlayers.forEach((player) => {
      ...
      gameData?.forEach((week) => {
        week.players.forEach((playerDetails) => {
          playerDetails.player.includes(player)
            ? playerMatches.matchesPlayed++
            : null;
        });
      });

      playersMatchesPlayed.push({
        player: playerMatches.player,
        matchesCount: playerMatches.matchesPlayed,
      });
    });
    ...
    uniquePlayers.forEach((value) => {
      ...
      gameData.forEach((temp) => {
        ...
        temp.players.filter((x) => x.player === value)[0]
          ? (playerIsInWeek = true)
          : (playerIsInWeek = false);

        if (playerIsInWeek) {
          const length = temp.players.filter((x) => x.player === value)[0]
            .rating?.length;
          temp.players
            .filter((x) => x.player === value)[0]
            .rating.forEach((x) => {
              averageRating.push(x.rating);
              let data1 = playerWeeklyData.find(
    ...
    playersMatchesPlayed.forEach((playerMatchesDetails) => {
      playerData.forEach((playerDataWithRatings) => {
        playerDataWithRatings.player.includes(playerMatchesDetails.player)
          ? (playerMatchesDetails['overallRating'] =
              playerDataWithRatings.overallRating)
          : null;
      });
    });

    this.optimizeDataForChart(playerWeeklyData, gameData);
    this.tableRawData = playersMatchesPlayed;
  }

  generateMatchHistoryTable(data: any) {
    let weeksArray = data.weeksArray;

    let matchHistoryData = weeksArray.map((matchData) => {
      return {
    ...
    this.matchHistoryData = matchHistoryData.sort((a, b) => a.date - b.date);
  }
  ...
  sortBy(property: any) {
    ...
    this.tableRawData = [
      ...this.tableRawData.sort((a: any, b: any) => {
  ...
  optimizeDataForChart(data: any, gameData: any[]) {
    data.forEach((element) => {
      ...
      const weeklyData = data1.reduce((a, b) => {
        ...
        return (
          found
            ? found.rating.push(b.rating)
            : a.push({ ...b, rating: [b.rating] }),
          a
        );
      }, []);
    ...
    const xAxisWeeklyCount = xAxisLabelsV2.reduce(
      (element, acc) => (element > acc ? element : acc),
      xAxisLabelsV2[0]
    );
